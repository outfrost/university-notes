...
--------------------|-----------------------------------------------------------|---------------------------
26.02.2018 07:30	| Podstawy konfiguracji i zabezpieczania przełączników		| Iwo Bujkiewicz (226203)

### Wyniki realizacji zadań

Pakiety `ping` z powodzeniem pokonywały trasę od `PC-A` do `S1`, od `PC-A` do `R1` i od `S1` do `R1` oraz z powrotem.

### Odpowiedzi na pytania

1. Dlaczego powinno konfigurować się linie vty na przełączniku?
	
	Aby mieć możliwość bezpiecznej konfiguracji przełącznika bez bezpośredniego połączenia konsoli szeregowej.

1. Dlaczego zmienia się domyślny VLAN 1 na inny?

	Aby nie był możliwy dostęp do wirtualnego interfejsu zarządzania przełącznikiem z każdego portu przełącznika.

1. Jak można zapobiec wysyłaniu haseł jawnym tekstem?

	Włączając w konfiguracji przełącznika szyfrowanie haseł.

1. Dlaczego konfiguruje się statyczny adres MAC na portach przełącznika?

	W celu zdefiniowania, jakie komputery mogą łączyć się z konkretnymi portami przełącznika.

1. Dlaczego włącza się bezpieczeństwo portów na przełączniku?

	Aby ograniczyć maksymalną liczbę adresów MAC przypisanych do jednego portu przełącznika i uniemożliwić niektóre niepożądane konfiguracje sieci.

1. Dlaczego nieużywane porty przełącznika powinny być wyłączone?

	Aby niemożliwe było podłączenie do sieci przypadkowego nowego urządzenia bez zatwierdzenia tego faktu w konfiguracji przełącznika.
